...
const AddAnnex = ({ formData, updateFormData }) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'price' || name === 'advancePayment') {
      ...
      updateFormData({ annexDetails: { ...localFormData, [name]: formattedValue } });
      setLocalFormData({ ...localFormData, [name]: formattedValue });
    } else {
     updateFormData({ annexDetails: { ...localFormData, [name]: value } });
  ...
  const handleQuickFillTitle = () => {
    const quickFillTitle = `${localFormData.bedrooms} bedroom annex in ${formData.placeType} for Rs.${localFormData.price} (per Month)`;
  ...
  const handleQuickFillDescription = () => {
    const quickFillDescription = `This ${localFormData.bedrooms}-bedroom annex offers great amenities and is located in ${formData.city}. Priced at Rs.${localFormData.price} per month, it's ideal for families or groups.`;
