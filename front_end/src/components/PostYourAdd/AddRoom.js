...
const AddRoom = ({ formData, updateFormData }) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'price' || name === 'advancePayment') {
      ...
      updateFormData({ roomDetails: { ...localFormData, [name]: formattedValue } });
      setLocalFormData({ ...localFormData, [name]: formattedValue });
    } else {
     updateFormData({ roomDetails: { ...localFormData, [name]: value } });
  ...
  useEffect(() => {
    if (formData && formData.roomDetails) {
      setLocalFormData({ ...formData.roomDetails });
    }
  }, [formData]);

  const handleQuickFillTitle = () => {
    const quickFillTitle = `One bedroom in ${formData.placeType} for Rs.${localFormData.price} (per Month)`;
    setLocalFormData({ ...localFormData, title: quickFillTitle });
    updateFormData({
      roomDetails: { ...localFormData, title: quickFillTitle }
    });
  };
  
  const handleQuickFillDescription = () => {
    const quickFillDescription = `This one bedroom offers great amenities and is located in ${formData.city}. Priced at Rs.${localFormData.price} per month.`;
    setLocalFormData({ ...localFormData, description: quickFillDescription });
    updateFormData({
      roomDetails: { ...localFormData, description: quickFillDescription }
    });
  };
          ...
          <FormGroup row>
            ...
            <FormControlLabel
              control={
                <Checkbox
                  name="cooking"
                  checked={localFormData.facilities.cooking || false}
                  onChange={handleFacilityChange}
                  className="button-checkbox"
                  sx={{ display: "none" }} 
                />
